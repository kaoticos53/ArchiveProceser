...
> - Nota posterior (fase 3I): el escritor dejó de escapar los caracteres no ASCII, así que volver a generar este testigo produce **el mismo documento** con otro texto (acentos sin escapar). El archivo comprometido se ha dejado intacto —la comparación es por documento, que es lo que la prueba fijó— y por eso la afirmación de arriba sobre el hash vale para el escritor de entonces, no para el de hoy.
...
## Fase 3I — Un archivo reparado converge (2026-09-22, rama `feature/crossplatform-avalonia`)

**Motivo.** P8 dejó la reparación y la versión, y con ellas una promesa escrita que nadie podía cumplir: `DeclareCurrent` no pisa una versión declarada «hasta que alguien lo repare», y **reparar no era una operación que existiera** —no había dónde decir que la reparación se había aplicado—. Al medir el camino completo aparecieron dos cosas: una premisa que era **falsa** y un defecto que era **peor** que la premisa.

La premisa falsa: la app **ya** convergía. Guardar exporta el lienzo, y el export es un grafo nuevo, así que el archivo anterior se guarda declarándose actual *porque el grafo que se escribe no es el que se leyó*. La convergencia dependía del escritor, no del archivo: el campo describía a quien escribía, no lo que el archivo era.

El defecto real, y es el que da sentido al trabajo: el camino que **sí** escribe el grafo que se leyó —`SaveWorkflowAsync`/`SerializeGraph` sobre lo que devolvió el lector, o `ToJson`— declaraba el formato actual un grafo que **no llevaba dentro lo que se había recuperado**, porque la reparación vivía sólo en los nodos del lienzo. El archivo reabierto se quedaba con **0 cables**: el formato actual no se repara, y los puertos recuperados no estaban en el archivo. La mutación que quita la siembra en el grafo lo reproduce con su diagnóstico exacto (`Expected reopenedEditor.Connections to contain 2 item(s) …, but found 0`). Es decir: la mitad que faltaba no era un detalle de coherencia, era la pérdida de datos que la propia regla decía evitar.

**Arreglo, en las dos mitades de la misma regla.** (1) `WorkflowFormat.DeclareRepaired` —la operación que faltaba—: aplicada la reparación, el grafo se declara lo que ahora es, y no baja versiones, porque un archivo posterior no lo repara esta versión. La llama `WorkflowGraphSerializer.Import`, que es el único sitio que aplica el plan, y lo hace cuando ya lo aplicó, no antes.

(2) `WorkflowFormat.NoteSource` y `WorkflowGraph : IJsonOnDeserialized`: todo grafo que llega de un texto recuerda de qué versión viene, y un archivo que no declaraba ninguna se anota con la del formato anterior al versionado (`WorkflowFormat.UndeclaredSchema`, que **nadie escribe**: es lo que el lector anota). Con eso `Schema` nulo significa una sola cosa —«no viene de ningún archivo»— y no dos, y `DeclareCurrent` puede seguir sin pisar nada: guardar un archivo anterior que nadie reparó lo escribe **como lo que es**, en vez de declararlo actual y enterrar su reparación. Lo hace el **modelo** al deserializarse y no cada lector porque los lectores son tres —el servicio de guardado, `FromJson` y cualquier camino que se añada— y el que se olvide **no falla**: guardaría un archivo anterior declarándolo actual.

(3) Lo que se recuperó se queda en el **grafo**, no sólo en el lienzo: `Import` escribe los puertos recuperados también en el DTO. Es la mitad que hace honesta la declaración —declarar actual un grafo sin la reparación dentro *es* la pérdida de arriba—, y para poder escribirla sin inventar nombres, los de las claves de la memoria de puertos (`RememberedInputPorts`/`RememberedOutputPorts`) y su formato —lista separada por `;`— pasan del nodo al **contrato** (`ISubflowNode`), que ya era su dueño semántico: tienen **dos** escritores con motivos distintos —el nodo al materializar, y quien reconstruye un archivo anterior— y dos definiciones del mismo nombre acabarían divergiendo.

**Decisiones que conviene conocer.** (1) La declaración la hace quien **repara**, no quien escribe: por eso vive junto al plan y no en `DeclareCurrent`, que no puede saber si el grafo que le dan ya se reparó. (2) `DeclareRepaired` no baja versiones: un archivo `v3` leído por esta versión conserva su versión aunque su grafo pase por el editor. (3) La anotación del origen es del modelo y no del lector, y es **idempotente** (sólo rellena lo que falta), de modo que un archivo posterior conserva la suya: aquí no se inventa ninguna versión, se escribe la que el archivo ya decía por omisión. (4) El orden importa y está en el código: primero se escribe lo recuperado en el grafo, después se declara reparado; al revés, la declaración iría por delante de lo que la sostiene.

> **Evidencia**
> - `WorkflowRepairDeclarationTests` (9 pruebas). **El origen:** un grafo construido en memoria no declara versión, y uno leído de un archivo sin versión la recuerda —comprobado por los **tres** lectores (`LoadWorkflowAsync`, `FromJson` y `DeserializeGraph`)—, mientras que uno de un archivo posterior conserva la suya. **Escribir sin reparar:** el archivo escrito declara el formato anterior y **no** el actual, y su reparación sigue calculándose desde sus propias aristas, que es lo que se perdería. **Reparar:** el grafo queda declarado actual **y** con los puertos recuperados en sus parámetros; el archivo guardado se reabre con los dos cables y sin volver a repararse; y el archivo reparado es **estable** —volver a abrirlo y guardarlo da el mismo texto—, que es la definición de haber convergido. **Y no baja versiones:** declarar la reparación de un grafo `v3` lo deja en `v3`.
> - **Los ejemplos que se entregan**: dos pruebas nuevas en `WorkflowExamplesValidationTests` —cada ejemplo declara la versión vigente y es lo que el escritor produce; cada uno se abre en el editor sin perder un nodo ni un cable—, y los 41 archivos reescritos con el camino de la aplicación.
> - **Verificación por mutación (cinco, y las cinco fallan como deben):** devolverle `: IFlowNode` al ejemplo de `api_reference.md` deja en rojo la guardia de la documentación nombrándolo; quitar la anotación del origen (dejar `OnDeserialized` vacío) deja en rojo **sólo** las tres pruebas del origen y de escribir sin reparar; quitar `DeclareRepaired` de `Import` deja en rojo **sólo** las dos que exigen la convergencia; **dejar la reparación sólo en el nodo** —sin escribirla en el grafo— deja en rojo esas dos, y la segunda con la pérdida medida: **0 cables** al reabrir; y declarar actual sin mirar la versión deja en rojo la prueba del archivo posterior **y** una de `NewerFormatProtectionTests`, que es la que custodia el aviso de 3A. Todas revertidas (`grep` sin residuos).
> - Verificación: `dotnet build FileFlow.slnx` sin errores ni advertencias y `dotnet test` con **1470 superadas / 0 fallos / 1 omitida** (1459 del baseline de 3E-G3 + las 9 de la fase + las 2 de los ejemplos).

**Los ejemplos que se entregan, con el ciclo nuevo.** Los 40 flujos del catálogo (`docs/examples/**`) y el `docs/flujo_test.json` de la raíz se reescribieron por el **camino real de la aplicación** —abrir (que repara y declara) y guardar—, así que declaran el formato actual y son lo que el escritor produce. Medido antes de reescribirlos: cada uno vuelve del editor con **los mismos nodos y los mismos cables** (`nodos=N->N aristas=N->N` en los 41), y el texto es estable al volver a escribirlo. Dos pruebas nuevas lo fijan para siempre en `WorkflowExamplesValidationTests`: cada ejemplo declara la versión vigente y es lo que el escritor produce, y cada uno se abre en el editor **sin perder un nodo ni un cable** (con el informe de reconstrucción a la vista). `docs/flujo_test.json` se actualizó de formato pero queda declarado **fuera de esa validación**: sus parámetros (`DestinationFolder`, `CleanWrapper` en el nodo de descompresión) no los declara ningún nodo hoy y su ruta es absoluta de otra máquina, así que validarlo obligaría a inventarle la traducción a los nombres vigentes.

**Un efecto que apareció al hacerlo, y que se corrigió.** Al reescribir los ejemplos se vio que el archivo de flujo **escapaba los caracteres no ASCII** (`Optimizaci\u00F3n`), mientras el resto del JSON del producto no lo hace: `JsonDefaults` existe justo para evitar ese escape y tiene pruebas que lo fijan. Las opciones del formato ganan el mismo `Encoder`; el documento no cambia —nada guardado deja de leerse y los archivos que ya están en disco se leen igual— y la comparación de testigos sigue siendo por documento, así que el testigo comprometido no se toca.

**La documentación viva, puesta al día.** El ciclo del formato quedó escrito donde se lee: una sección propia en `architecture.md` (versión, reparación, convergencia y los otros artefactos), la tabla de API en `api_reference.md` —cuyo ejemplo de nodo seguía con `: IFlowNode`, `NodePinDefinition` y `ValidateConfiguration`, que ya no existen—, el apartado del **estado de diseño** y la regla de la versión en `nodes/CREATING_NODES.md`, el comportamiento en los manuales de usuario (ES/EN, incluidos los de principiantes), el catálogo de pruebas, la guía de contribución, el resumen de arquitectura para agentes, el walkthrough y el README de los ejemplos. La guardia que vigila el ejemplo de la guía vigila ahora también los bloques de `api_reference.md` (mutación: devolverle `: IFlowNode` al ejemplo la pone en rojo nombrándola).

**Límite declarado.** (1) No se convierte lo que ya está en disco: un archivo anterior se repara al abrirlo y converge **cuando el usuario lo guarda**; abrir y no guardar no escribe, así que en disco ese archivo sigue anterior y se volverá a reparar la próxima vez. (2) Declarar el formato actual cierra el hueco del **formato**, no el del **dato**: los casos de un switch sin `CasesJson`, una definición incrustada perdida y un puerto sin cable siguen sin recuperarse —nunca estuvieron en el archivo— y un flujo así se guarda como actual con esos huecos dentro, que es el límite que P8 ya dejó escrito con prueba. (3) El paquete del portapapeles no entra aquí: lleva su propio nombre y su propia versión, y no es el archivo de flujo.
