...
6. **Resolución de Incidencias Críticas en el Cliente Multiplataforma**:
   - **Arranque Determinista y Carga HTTP en Photino**: Sustituido el temporizador arbitrario por `await app.StartAsync()` en hilo principal `[STAThread]` y forzada la carga vía URL HTTP (`window.Load(new Uri(serverUrl))`). Se descubrió que Photino al cargar `file:///.../index.html` provocaba que WebView2 (Chromium) bloqueara por CORS los scripts de tipo `<script type="module">` generados por Vite, impidiendo la ejecución de JavaScript y dejando la ventana en negro. La carga vía HTTP Kestrel desbloqueó por completo la carga de JavaScript con respuesta HTTP 200 para todos los assets.
