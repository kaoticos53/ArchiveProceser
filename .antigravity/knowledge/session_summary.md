...
- **Hitos Activos y Recientes (Septiembre 2026)**:
  ...
  - **84. Barra de Menú Completa, Diálogos Modales Especializados y Parámetros Ricos en la Edición Multiplataforma (Web & Photino)**:
    ...
    - Inspector de parámetros (`InspectorDrawer.tsx`) enriquecido con soporte de todos los editores (`text`, `number`, `slider`, `toggle`, `select`, `folderpath`/`filepath`, `multilinetext`) y botones destacados de "Diseñadores y Asistentes".
    - `FileFlow.Server` actualiza `/api/nodes/catalog` para serializar los `ParameterDescriptors` y `CustomActions` del SDK. Compilación limpia y 833/833 pruebas unitarias superadas al 100%.
